...
import { PANELS, PANEL_IDS, findPlugin, panelSrc } from './nexusPanels'
...
export default function App() {
  ...
  // Asked once. The film and the deck are served by the plugin, so a page opened in a
  // plain browser has to be told that rather than shown two frames that will never load.
  useEffect(() => {
    const abort = new AbortController()
    findPlugin(abort.signal).then((base) => setPluginBase(base ?? false))
    return () => abort.abort()
  }, [])
  ...
  return (
    <main className="visualizer">
      ...
      <nav className="view-switcher" aria-label="Diagram view">
        ...
        {PANELS.map((panel) => (
          <button
            key={panel.id}
            type="button"
            className={activeView === panel.id ? 'view-switcher__active' : ''}
            aria-pressed={activeView === panel.id}
      ...
      {/*
        Mounted for as long as the tab is open, and hidden rather than unmounted, because a
        film takes most of a minute to generate and switching to the Code tree to check
        something must not throw that away. The IDE reaches these frames by name, so an
        unmounted one would also mean a reply with nowhere to land.
      */}
      {PANEL_IDS.includes(activeView) && pluginBase === false && (
        <section className="panel-workspace panel-workspace--empty">
          <div className="panel-missing" role="status">
            <strong>Open this inside the IDE</strong>
            <span>
              The film and the deck are generated by the Nexus plugin, so they need the IDE
              running behind this page. The other views work here on their own.
            </span>
          </div>
        </section>
      )}

      {pluginBase && PANELS.map((panel) => (
        <section
          key={panel.id}
          className="panel-workspace"
          aria-label={panel.label}
          hidden={activeView !== panel.id}
        >
          <iframe
            className="panel-frame"
            name={panel.frame}
            title={panel.blurb}
            src={panelSrc(panel, projectParams)}
          />
        </section>
      ))}
