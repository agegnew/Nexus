      ...
      <nav className="view-switcher" aria-label="Diagram view">
        {VIEWS.map((view) => (
          <button
            key={view.id}
            type="button"
            className={activeView === view.id ? 'view-switcher__active' : ''}
      ...
      {PANEL_IDS.includes(activeView) && pluginBase === false && (
        <section className="panel-workspace panel-workspace--empty">
          <div className="panel-missing" role="status">
            ...
            <span>
              The film, the deck and the trust picture all come from the Nexus plugin, so they
              need the IDE running behind this page. The other views work here on their own.
            </span>
          </div>
        </section>
      )}
